...
public class Tagger {
	...
	public static String uniqueAccessKey="IQAOMLmju2ovRwabGhRocN7OiKnhyMCX";		
	private File input;
	private File output;
	...
	public static void main(String[] args) throws IOException {
		//verifyArgs(args);
		Tagger httpClientPost = new Tagger();
		CreateObituariesFromCSV.openCSVRead("/Users/himanshumisra/Downloads/meaning.csv");
		...
		if (dir.isDirectory()) { 
			for (final File f : dir.listFiles()) {
				try {
					...
					if(s.charAt(0)!='.')
					{
					File newfile =new File(directoryPath+"/" +s.split("\\.")[0]+ ".n3");
					if(f.renameTo(newfile)){
						
					}else{
						System.out.println("Rename failed for "+ f.getAbsolutePath());
					}
					}
			...
			System.out.println("Rename Process Complete");
		}
	}
